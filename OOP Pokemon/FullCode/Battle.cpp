...
int Battle::Attack(Pokemon attacker, Pokemon defender) {
    ...
    if ((attacker.getType() == "Water" && defender.getType() == "Fire") ||
        ...
        damage *= 2;
        cout << attacker.getName() << " strictly attacks for " << attacker.getAttack() * 2 << " damage to " << defender.getName() << endl;
    } else {
        cout << attacker.getName() << " attacks for " << attacker.getAttack() << " damage to " << defender.getName() << endl;
    }
    return damage;
}
