...
Player::Player(){
    player_list = new Pokemon[len_list];
};

void Player::AddPokemon(string filename, string PokeName) {
    if (current == len_list) {
        cout << "Cannot get the new pokemon! Maximum " << len_list << " pokemons in your pocket!!!" << endl;
        return;
    }
...
void Player::SavePokeListToFile() {
    ...
    for (int i = 0; i < current; i++) {
        ...
        string pokemonInfo = player_list[i].getName() + " "+ player_list[i].getType() 
                            + " " + player_list[i].getSound() + " " + to_string(player_list[i].getHP()) 
                            +  " " + to_string(player_list[i].getAttack()) + " " + to_string(player_list[i].getSpeed());
...
Pokemon Player::ChoosePokemon() {
    ...
    for (int i = 0; i < current; i++) {
            ...
            cout << "Type: " << player_list[i].getType() << "  Health: " << player_list[i].getHP() << "  Attack: " << player_list[i].getAttack() << "  Speed: " << player_list[i].getSpeed() << endl;
        }
    while (true) {
...
string Player::get_trainer_name() {
    ...
    return name; // Return 0 to indicate success
}

Pokemon* Player::GetPlayerList() {
    return player_list;
}
void Player::set_len_list(int len) { len_list = len;}
int Player::get_len_list() { return len_list;}
